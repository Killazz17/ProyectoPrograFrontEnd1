...
public class LoginView extends JFrame {
    ...
    public LoginView() {
        ...
        setupFrame();
        setupListeners();
        // Apply image icon safely after UI components are initialized
        applyImageSafely();
    }
    ...
    /**
     * Intenta aplicar el icono de imagen al ImageLabel de manera segura.
     * Busca en varias rutas comunes y sólo crea ImageIcon cuando la URL no es null.
     */
    private void applyImageSafely() {
        if (ImageLabel == null) return;

        // Rutas probables donde la imagen podría estar empaquetada
        String[] candidatePaths = new String[]{
                "/images/logo.png",
                "/logo.png",
                "/Presentation/Views/logo.png",
                "/assets/logo.png"
        };

        java.net.URL imgLocation = null;
        for (String path : candidatePaths) {
            imgLocation = LoginView.class.getResource(path);
            if (imgLocation != null) break;
        }

        if (imgLocation != null) {
            ImageLabel.setIcon(new ImageIcon(imgLocation));
        } else {
            // Evita NullPointerException asignando un icono vacío
            ImageLabel.setIcon(new ImageIcon());
            System.err.println("Aviso: imagen de login no encontrada en classpath; usando icono vacío.");
        }
    }
